...
class Consorcio {
    ...
    valorPrest: number; //3
    vPago: number; //4
    saldoDevedor: number; //5

    constructor(p_nPrestacoes:number, p_prestPagas:number, p_valorPrest:number, p_vPago: number, p_saldoDevedor:number) {
        this.nPrestacoes = p_nPrestacoes;
        this.prestPagas = p_prestPagas;
        this.valorPrest = p_valorPrest;
        this.vPago = p_vPago;
        this.saldoDevedor = p_saldoDevedor;
    }
